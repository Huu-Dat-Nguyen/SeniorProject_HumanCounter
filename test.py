...
class FootageListView(BoxLayout):
    ...
    def go_to_video_player(self, filepath):
        video_player_screen = VideoPlayerScreen(filepath)
        App.get_running_app().root.add_widget(video_player_screen)
        App.get_running_app().root.current = video_player_screen.name
...
class VideoPlayerScreen(Screen):
    def __init__(self, video_path, **kwargs):
        ...
        self.add_widget(Label(text=f"Now playing: {video_path}"))  # Placeholder for the video player UI
